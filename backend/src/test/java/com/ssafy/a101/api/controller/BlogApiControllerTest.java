...
class BlogApiControllerTest {
    ...
    @Autowired
    protected ObjectMapper objectMapper; // 직렬화 역직렬화를 위하 클래스

    @Autowired
    private WebApplicationContext context;

    @Autowired
    private BlogRepository blogRepository; // blogRepository 주입

    @BeforeEach // 테스트 실행 전 생성
    ...
    @DisplayName("addArticle : 글 추가를 성공하겠습니다")
    @Test
    public void addArticle() throws Exception{
        ...
        final AddAriticleRequest userRequest = new AddAriticleRequest(title, content);
    ...
    @DisplayName("fingArticle :  블로그 글 조회에 성공한다")
    @Test
...
//                .andExpect(status().isOk()); //  글이 있어야 삭제가 됨으로 없으면 안된다뜸!
//
//        // then
//        List<Article> articles = blogRepository.findAll();
//
